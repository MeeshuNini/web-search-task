    ...
    def crawl(self, url, base_url=None):
        ...
        try:
            ...
            for link in soup.find_all('a'):
                href = link.get('href')
                if href:
                    # Normalize the href using urljoin for both absolute and relative URLs
                    full_url = urljoin(base_url or url, href)
                    # Only crawl internal links (i.e., same domain)
...
# ------------------- RUNNING ONLY ONE MAIN ENTRY -------------------

if __name__ == "__main__":
    # Choose whether to run unit tests or main application
    # Uncomment the one you want to run

    unittest.main()  # For unit testing
    # main()         # For running the actual web crawler
